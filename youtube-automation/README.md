...
- **팩트체크 2차 검증** (구현됨): `config.yaml`의
  `script_generation.fact_check_enabled: true`로 켜면, 대본 생성 직후
  별도 Claude 호출(`fact_checker.py`)로 "이 문장이 기사 출처로
  뒷받침되는가/인신공격성 표현은 없는가"를 재검증합니다. 결과는
  `script.json`의 `fact_check` 필드에 기록되고, 문제가 있으면 콘솔에
  경고가 출력됩니다. **사람의 최종 검수를 대체하지 않으며**, API 호출이
  1번 더 늘어나 비용/시간이 증가하므로 기본값은 꺼져 있습니다.
- **썸네일 자동 생성** (구현됨): `thumbnail.py`가 영상과 같은 배경색/폰트로
  1280x720 썸네일을 자동 생성합니다(`output/.../thumbnail.jpg`).
...
- **다국어 자동 생성**도 파이프라인에 단계 추가가 용이한 구조입니다.
